...
macro_rules! register_boxed_big_array_serialization {
  ($($len:expr,)+) => {
      $(
              ...
              where T: Default + Copy + Serialize + Deserialize<'de>
          {
                  ...
                  where D: Deserializer<'de>
              {
                      ...
                      where T: Default + Copy + Deserialize<'de>
                  {
                      ...
                      {
                          let mut arr : Box<[T; $len]> = Box::new(unsafe { std::mem::MaybeUninit::uninit().assume_init() });
                          for i in 0..$len {
...
register_boxed_big_array_serialization! {
  256,
}
